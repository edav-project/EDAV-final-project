...
GDP_sub<-subset(GDP,select=-c("X","GeoFIPS","Region","TableName","IndustryId","IndustryClassification"))
...
```{r}
library("ggmap")
TimePlot<-function(gun_vio,back_check,population,state)
{
  gun_vio$date<-as.Date(gun_vio$date)
  gun_vio_tp<-gun_vio[which( gun_vio$state==state), ]
  gun_vio_tp<-subset(gun_vio_tp,select=c("date","state","n_killed","n_injured"))
  gun_vio_tp["year"]<-format(as.Date(gun_vio_tp$date, format="%d/%m/%Y"),"%Y")
  gun_vio_tp["month"]<-format(as.Date(gun_vio_tp$date, format="%d/%m/%Y"),"%m")
  ...
  merge(gun_vio_month,population_sub,by.x="year",by.y="Year")
...
TimePlot(gun_vio,back_check,population,"California")
...
back_check$month<-as.character(back_check$month)
back_check["year"]=str_sub(back_check$month,1,4)
Parcoord<-function(gun_vio,back_check,uem_month,GDP,year)
{
  year=2018
  year<-as.character(year)
  back_check_year<-back_check[ which(back_check$year==year), ]
...
Parcoord(gun_vio,back_check,uem_month,GDP,"2016")
```
