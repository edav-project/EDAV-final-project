...
p <- ggplot(gun_vio_day_new)+
  geom_line(aes(x=adjusted_date,y=incident_number,color="incident_number"))+
  stat_smooth(aes(x=adjusted_date,y=incident_number,color="incident_number"))+
  geom_line(aes(x=adjusted_date,y=sum_killed,color="sum_killed"))+
  stat_smooth(aes(x=adjusted_date,y=sum_killed,color="sum_killed"))+
  geom_line(aes(x=adjusted_date,y=sum_injured,color="sum_injured"))+
  stat_smooth(aes(x=adjusted_date,y=sum_injured,color="sum_injured"))+
 scale_fill_discrete(name = "Variables", labels = c("incident_number", "sum_injured", "sum_killed"))+
  ggtitle("Time Series of Gun Violence")+
  xlab("date")
```
...
Parcoord<-function(year)
{
  ...
  # check_vio_uem_party<-merge(check_vio_uem,states_party,by.x='state',by.y='State')
  # 
  ...
  parcoords(check_vio_uem[c("state","checking_totals","gun_violence_death","GDP_avg")],rownames = FALSE,reorder = TRUE, brushMode="1D",alpha = .3, color = list(colorScale = "scaleOrdinal",colorBy = "state", colorScheme = "schemeCategory10"), withD3 = TRUE)
    ...
    # parcoords(check_vio_uem_party[c("Parties","checking_totals","gun_violence_death","GDP_avg")],rownames = FALSE,reorder = TRUE, brushMode="1D",alpha = .3, color = list(colorScale = "scaleOrdinal",colorBy = "Parties", colorScheme = "schemeCategory10"), withD3 = TRUE)
